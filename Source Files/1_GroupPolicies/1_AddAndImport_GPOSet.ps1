...
.DESCRIPTION
    ...
    - Validates the current working directory and navigates to the Group Policy backup folder (`C:\_psc\ADC_Setup\1_GroupPolicies`)
    - Reads and validates the XML manifest (`manifest.xml`) within the backup folder (`GPOSet_latest`)
    - Iterates through all listed GPOs, creating each policy in the domain if it doesn’t exist
...
function Import-GPOSet {
	# Log file path
    $logFile = "C:\_psc\ADC_Setup\Logfiles\AddAndImport_GPOSet.log"
	...
	if($location -ne "C:\_psc\ADC_Setup\1_GroupPolicies"){
		Write-Log " Changing location to (C:\_psc\ADC_Setup\1_GroupPolicies)"
		$location = "C:\_psc\ADC_Setup\1_GroupPolicies"
	}
	else{
		#Nothing to do
